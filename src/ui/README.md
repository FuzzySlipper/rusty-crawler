...
offered disabled while the product says no member may act. It reports the pacing on the same terms too:
...
product's `combat.turn-based`, `combat.turn-skip`, and `combat.turn-wait` actions. The panel runs no
