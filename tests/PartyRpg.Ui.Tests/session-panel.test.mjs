...
function snapshot(mode, seconds = 0, steps = 0, updates = 0) {
  return {
    composition: { ruleset: 'mightandmagic7', title: 'Might and Magic VII: For Blood and Honor' },
...
test('the companion holds no state and starts no timer', () => {
  ...
  try {
    ...
    assert.deepEqual(readPanel(h).values, ['running', '3.0 s', '180', '182']);
